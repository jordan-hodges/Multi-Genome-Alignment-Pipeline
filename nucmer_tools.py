...
def size_sort_contigs(cid2seq, minKb = 20):
	...
	for cid in id2seq.keys():	
		...
		if size in size2contigs.keys(): size2contigs[size].append(cid)
		else: size2contigs[size] = [cid]
		
	#then sort the contigsizes
	sizes = list(size2contigs.keys())
	sizes.sort()
		...
		for y in range(len(list_of_genomes)):
			...
			if coords_fname_XY not in coords_files: # we don't have output for this comparison, we should have output for the reverse comparison
				...
				for line in lines[5:]: #skip header:
					...
					if Xscaffold in scaffold2starts2new_lines.keys():
						if start in scaffold2starts2new_lines[Xscaffold].keys():
				...
				for Xscaffold in Xscaffold_list: #use order of scaffolds from fasta
					if Xscaffold in scaffold2starts2new_lines.keys():
						starts = list(scaffold2starts2new_lines[Xscaffold].keys())
...
def assign_scaffold2scaffold(coordsfilename, Rscaffolds, promer = False):
	...
	for line in lines:
		...
		if Rscaffold in Rscaffolds:
			#print data
			if Rscaffold not in Rscaffold2coordslines.keys():
				Rscaffold2coordslines[Rscaffold] = [line]
			else:	Rscaffold2coordslines[Rscaffold].append(line)
			
			
			if Qscaffold not in Qscaffold2Rscaffold.keys():
				Qscaffold2Rscaffold[Qscaffold] = {}
				for Rscaffold_tmp in Rscaffolds: 
					Qscaffold2Rscaffold[Qscaffold][Rscaffold_tmp] = [[],[]] # two values, one for the + and one for the - strand
				
			# Collect regions that correspond to alignments between the two scaffolds
			# + strand
			if int(data[0]) < int(data[1]):
				if int(data[3]) < int(data[4]): 
					Qscaffold2Rscaffold[Qscaffold][Rscaffold][0].append((int(data[0]), int(data[1])))
				else:	Qscaffold2Rscaffold[Qscaffold][Rscaffold][1].append((int(data[0]), int(data[1])))
			# - strand 
			else:
				if int(data[3]) < int(data[4]): 
					Qscaffold2Rscaffold[Qscaffold][Rscaffold][1].append((int(data[1]), int(data[0])))
				else:	Qscaffold2Rscaffold[Qscaffold][Rscaffold][0].append((int(data[1]), int(data[0])))

	Qscaffold2direction  = {}
	Qscaffold2bestmatchingRscaffold = {}
	
	
	for Qscaffold in Qscaffold2Rscaffold.keys():
		maxlength  = 0
		bestmatch  = ''
		maxlength0 = 0
		maxlength1 = 0
		...
		for Rscaffold in Rscaffolds:
			# non-overlapping aligned regions on + strand
			regions0 = blast_tools.merge_overlapping_regions(Qscaffold2Rscaffold[Qscaffold][Rscaffold][0], min_overlap = 1, minusstrand = False)
			# non-overlapping aligned regions on - strand
			regions1 = blast_tools.merge_overlapping_regions(Qscaffold2Rscaffold[Qscaffold][Rscaffold][1], min_overlap = 1, minusstrand = False)
			
			# cumulative length of non-overlapping regions
			# + strand
			length0 = 0
			for (s,e) in regions0:
				length0 += e-s
			...
			# check if this R scaffold has the longest aligned regions
	...
	return Rscaffold2coordslines, Qscaffold2Rscaffold, Qscaffold2direction, Qscaffold2bestmatchingRscaffold
...
def get_order_and_direction_of_Qscaffolds__to_get_a_diagonal(coordsfile, Rscaffolds, outfilename=None, promer = False):
	...
	for Rscaffold in Rscaffolds:
		...
		if Rscaffold in Rscaffold2coordslines.keys():
			...
			for line in lines:
				...
				if Qscaffold not in mappedscaffolds:
					...
					if Qscaffold in Qscaffold2bestmatchingRscaffold.keys():
